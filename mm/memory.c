...
__asm__("pushl %%ecx; pushl %%edi; pushl %%esi; addl $12, %%esp":);\
__asm__("cld ; rep ; movsl": :"S" (from),"D" (to),"c" (1024)); \
__asm__("subl $12, %%esp; popl %%esi; popl %%edi; popl %%ecx":)
...
unsigned long put_dirty_page(struct task_struct * tsk, unsigned long page, unsigned long address)
{
	...
	if (PAGE_PRESENT & *page_table)
		page_table = (unsigned long *) (PAGE_MASK & *page_table);
	else {tmp = get_free_page(GFP_KERNEL);
...
// 	if(address== 0x60090000 || address== 0x60091000 || address== 0x60093000)
// 		printk(" no pg: %lx ", address);
		return;
	}
	if (tsk != current)
		goto ok_no_page;
...
asmlinkage void do_page_fault(struct pt_regs *regs, unsigned long error_code)
{
	...
	if (address < TASK_SIZE) {
		...
		return;
	}/*printk(" at address %08lx > %08lx ",address, TASK_SIZE);*/
	address -= TASK_SIZE;
...
unsigned long __bad_pagetable(void)
{
	...
	__asm__ __volatile__("pushl %%edi; pushl %%ecx":);
	__asm__ __volatile__("cld ; rep ; stosl; popl %%ecx; popl %%edi":
...
unsigned long __bad_page(void)
{
	...
	__asm__ __volatile__("pushl %%edi ; pushl %%ecx ; addl $8, %%esp":);
	__asm__ __volatile__("cld ; rep ; stosl":
		:"a" (0),
		 "D" ((long) empty_bad_page),
		 "c" (PTRS_PER_PAGE)
		);
	__asm__ __volatile__("subl $8, %%esp ; popl %%ecx ; popl %%edi":);
